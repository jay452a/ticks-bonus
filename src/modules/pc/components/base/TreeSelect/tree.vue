<template>
  <div class="ats-tree"
    ...
    <div class="ats-input"
      ...
      <div class="ats-input-multiple el-input__inner"
           v-if="this.multiple"
           :style="{width: inputWidth +'px'}">
        <i class="el-input__icon el-icon-caret-bottom"
        ...
        <div class="ats-labels">
          <div class="el-select__tags"
               @click.prevent="handleCloseTree(true)"
               :style="{width: (inputWidth - 40)+'px'}">
            <el-tag v-for="item in checkedItems"
                    :key="item.value"
                    :closable="true"
                    type="primary"
          ...
          <input :style="{width: (inputWidth - 40)+'px'}"
                 ref="multipleInput"
                 type="text"
                 :placeholder="placetext"
                 v-model="treeSelected"
    ...
    <el-scrollbar v-show="treeVisible"
                  class="ats-tree-scrollbar"
                  ref="treeScrollbar">
      <div class="ats-tree-wrapper">
        <ul class="ats-tree-nodes">
          <tree-node v-for="child in treeNodes[treeProps.children]"
                     :node="child"
                     :key="child.value"
                     :multiple="multiple"
...
import { objArrDeepCopy } from 'common/js/tools';
  ...
  methods: {
    ...
    handleNodeClick(node, event) {
      ...
      if (this.multiple) {
        ...
        this.$emit('setSelectedId', node.value);
      }
    },
    setDefaultValue() {
      if (!this.multiple) {
    ...
    setCheckedKeys(val) {
      ...
      val &&
        val.forEach(function(value) {
          self.findTreeItem(value, treeNodes);
        });
      this.getNotExistedItem(val, this.checkedKeys);
    },
    findTreeItem(value, treeNodes) {
      let self = this;
      ...
      for (let i = 0; i < childNodes.length; i++) {
        if (childNodes[i].value == value) {
          ...
          self.findTreeItem(value, childNodes[i]);
        }
      }
    },
    ...
    handleDelItem(item, event) {
      ...
      this.checkedKeys.splice(this.checkedKeys.indexOf(item.value), 1);
...
.ats-tree {
  ...
  .el-input__inner {
    ...
    border-radius: 4px;
    border: 1px solid rgb(217, 194, 191);
  ...
  .ats-tree-scrollbar {
    ...
    position: absolute;
    z-index: 1001;
    border: solid 1px rgb(229, 212, 209);
    border-radius: 2px;
    background-color: #fff;
